...
- 多智能体协作：基于 LangGraph 编排任务分解、网页搜索、内容综合、质量评估等工作流，实现报告和 PPT 的自动化生成。
...
推荐使用 Python 3.10+，建议使用 Conda

```powershell
conda create -n smartfin python=3.11
...
```p网页搜索依赖laywright in浏览器：hromium
```
...
DEFAULT_LLM_MODEL=your_LLM_model
...
```text
report    生成研究报告（综合、日报、分析、研究类型）
analyze   金融数据分析，可选生成报告或 PPT
ppt       演示文稿生成
export    导出项目（PPTX、PDF、DOCX、Markdown）
iterate   迭代修改已有项目
ask       快速问答入口
status    查看运行配置状态
```
...
```powershell
python SmartFin.py report "2026年人工智能行业趋势分析"
python SmartFin.py report "新能源汽车产业链研究" --type research使用示例th deep 生成研究报告ts 30 -o html -v
python SmartFin.py report "企业数字化转型方案" --input-file .\docs\brief.docx
```
...
```powershell
python SmartFin.py analyze "分析2024年银行业营收趋势"
python SmartFin.py analyze "分析华为营收表现" --deliverable report -o html
python SmartFin.py analyze "分析新能源行业财务表现" --deliverable ppt -s business -n 12
python SmartFin.py analyze "仅输出结构化金融分析" --deliverable none
```
...
| 参数 | 说明 |
| --- | --- |
| `--dSmartFinble` / `-D` | `report`、`ppt`、`none` |
| `--depth` / `-d` | 搜索深度 |SmartFinax-results` / `-m` | 最大搜索结果数 |
| `--ppt-style` 常用参数：PPT 风格 |
| `--slides` / `-n` | PPT 页数 |
...
```powershell
python SmartFin.py ppt "2026年产品发布会"
python SmartFin.py ppt "公司年度经营复盘" --style business --slides 15 --theme blue
python SmartFin.py ppt "AI 在教育行业的应用" --speech生成es "面向高校教师的分享"
```
...
```powershell
python SmartFin.py iterate <project_id> "补充第三章的数据案例，并让结论更聚焦"

python SmartFin.py export <project_id> --type pdf
python SmartFin.py export <project_id> --type docx
python SmartFin.py export <project_id> --type pptx
python SmartFin.py export <project_id> --type md -o output.md
```
