...
class LangfuseMonitor:
    ...
    def _initialize(self):
        ...
        public_key = os.getenv("LANGFUSE_PUBLIC_KEY") or os.getenv("LANGFUSE_PUB_KEY")
        base_url = (
        ...
        if not secret_key or not public_key:
            ...
            logger.info(
                ...
                f"LANGFUSE_PUBLIC_KEY/LANGFUSE_PUB_KEY={'set' if public_key else 'missing'}"
            )
            return
        
        logger.info(
            f"Langfuse: LANGFUSE_BASE_URL={base_url}, "
            f"LANGFUSE_SECRET_KEY={'set' if secret_key else 'missing'}, "
            f"LANGFUSE_PUBLIC_KEY/LANGFUSE_PUB_KEY={'set' if public_key else 'missing'}"
        )
        ...
        try:
            ...
            self.enabled = True
            logger.info(f"Langfuse: {base_url}")
