...
class WebSearcher:
    ...
    async def search(
        ...
        try:
            ...
            if should_fetch_content:
                ...
                search_results = await self._fetch_full_content_with_httpx(search_results)
    ...
    async def _search_duckduckgo(
        ...
        """
        try:
            logger.info(f"[{self.name}] DuckDuckGo (httpx)")

            self.duckduckgo_searcher.topk = max_results

            search_results = await self.duckduckgo_searcher.search_with_httpx(
                query=query,
    ...
    async def _fetch_full_content_with_httpx(
        ...
        """
        URL - httpx + BeautifulSoup (no browser needed).

        Args:
            search_results: [{title, url, snippet}]

        Returns:
            [{title, url, snippet, full_content, images, has_full_content}]
        """
        logger.info(f"[{self.name}] {len(search_results)} URL (httpx)...")

        async def fetch_one(result: Dict[str, Any], idx: int, total: int) -> Dict[str, Any]:
            url = result.get("url", "")
            if not url:
                return {**result, "full_content": "", "images": [], "has_full_content": False}

            logger.info(f"[{self.name}]  ({idx+1}/{total}): {url}")
            ...
            try:
                ...
                for selector in ["article", "main", "[role='main']",
                    ...
                    if el:
                        full_content = el.get_text(separator="\n", strip=True)
                        break
                ...
                if self.extract_images:
                    for img_tag in soup.select("img"):
                        ...
                        w = int(img_tag.get("width") or img_tag.get("data-width") or 0)
                        h = int(img_tag.get("height") or img_tag.get("data-height") or 0)
                        if w < 200 and h < 200:
                            continue
                ...
                logger.info(f"[{self.name}]  ({idx+1}/{total}): {len(full_content)} , {len(images)} ")
                return {
                    **result,
                ...
                logger.error(f"[{self.name}]  ({idx+1}/{total}) {url}: HTTP {e}")
                return {
                    **result,
                ...
                logger.error(f"[{self.name}]  ({idx+1}/{total}) {url}: {e}")
                return {
                    **result,
                    "full_content": result.get("snippet", ""),
                    "images": [],
        ...
        tasks = [fetch_one(r, i, total) for i, r in enumerate(search_results)]
        results = await asyncio.gather(*tasks, return_exceptions=False)

        logger.info(f"[{self.name}]  {sum(1 for r in results if r.get('has_full_content'))}/{total} ")
        return list(results)
