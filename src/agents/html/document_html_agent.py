...
class DocumentHTMLAgent(BaseHTMLAgent):
    ...
    def _get_additional_template_dirs(self) -> List[Path]:
        """Get project-level document template directories."""
        return [
            Path(__file__).parent.parent.parent.parent / 'templates' / 'html' / 'document'
        ]
    ...
    def parse_content(self, content: str, metadata: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        # 
        title = metadata['title'] if 'title' in metadata else self._extract_title(content)
