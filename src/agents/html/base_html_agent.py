...
class BaseHTMLAgent(ABC):
    ...
    def __init__(
        ...
        additional_dirs_getter = getattr(self, "_get_additional_template_dirs", None)
        if callable(additional_dirs_getter):
            for extra_dir in additional_dirs_getter():
                extra_path = Path(extra_dir)
                if extra_path not in template_dirs:
                    template_dirs.append(extra_path)
