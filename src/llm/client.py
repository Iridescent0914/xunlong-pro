...
class LLMClient:
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        # --- 在这里增加节流阀 ---
        # 强制暂停 1.5 秒，确保请求不会密集并发打向阿里云 API
        ...
        try:
            ...
            # 429/超时由上层重试逻辑处理，不在此处打印 ERROR
            if "429" in error_str or "limit_requests" in error_str or "timeout" in error_str.lower() or "TimedOut" in type(e).__name__:
                pass  # 上层重试
            ...
            elif "base_url" in error_str.lower() or "connection" in error_str.lower():
...
JSON"""
    ...
    async def _openai_chat_completion(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 429 和超时归类为可重试错误，其他转为 ValueError
            error_str = str(e)
            is_retriable = (
                "429" in error_str
