...
SmartFin的迭代系统允许您精炼和修改生成的内容，而无需从头开始。
...
```bash
# 生成初始内容
python SmartFin.py report "AI趋势" --depth standard

# 迭代修改
python SmartFin.py iterate <project-id> "在结论中添加更多示例"
```
...
# 查看特定版本
python SmartFin.py view <project-id> --version 2

# 回滚到之前版本
python SmartFin.py rollback <project-id> --to-version 2

# 比较版本
python SmartFin.py diff <project-id> --from 1 --to 3
```

### 版本历史

```bash
python SmartFin.py history <project-id>
```
...
```bash
# 查看最后的更改
python SmartFin.py changes <project-id>
```
...
```bash
# 添加数据
python SmartFin.py iterate <report-id> \
  "添加2025年第一季度的财务数据"
...
```bash
# 角色发展
python SmartFin.py iterate <fiction-id> \
  "深化主角在第8-12章的动机"
...
```bash
# 幻灯片内容
python SmartFin.py iterate <ppt-id> \
  "简化第5-8张幻灯片的要点"
...
```bash
# 1. 生成初始报告
python SmartFin.py report "区块链技术" --depth standard
...
```bash
# 1. 生成初稿
python SmartFin.py fiction "赛博朋克推理" \
  --chapters 20 \
  --style mystery
...
# 版本管理
python SmartFin.py versions <project-id>
python SmartFin.py rollback <project-id> --to-version <N>
python SmartFin.py diff <project-id> --from <N> --to <M>
...
# 比较
python SmartFin.py compare <project-id> <other-project-id>
```
